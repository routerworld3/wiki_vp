...
## 3. Terminology and tenant settings structure
...
### 3.1 Where the settings live

**CTAS** — Cross-Tenant Access Settings. Located at **Entra admin center → External Identities → Cross-tenant access settings**. This is the blade that decides *which external tenants your users may reach* and *which external users may reach you*. It is separate from, and evaluated before, Conditional Access.

```mermaid
graph TD
    subgraph TA["Entra-A — HOME tenant (owns user + device)"]
        A0["External Identities →<br/>Cross-tenant access settings"]
        A0 --> AD["Default settings<br/>applies to every tenant<br/>not explicitly configured"]
        A0 --> AO["Organizational settings →<br/>entry for tenant B"]
        AO --> AOUT["OUTBOUND access ← the active tab here"]
        AOUT --> AOU["B2B collaboration → Users and groups<br/>which A users/groups may leave"]
        AOUT --> AOA["B2B collaboration → External applications<br/>which B apps they may reach"]
        AO --> AIN["INBOUND access<br/>(not used in this direction)"]
        AO --> ACL["Microsoft cloud settings<br/>cross-cloud partnering"]
    end

    subgraph TB["Entra-B — RESOURCE tenant (owns App-B + guest)"]
        B0["External Identities →<br/>Cross-tenant access settings"]
        B0 --> BD["Default settings"]
        B0 --> BO["Organizational settings →<br/>entry for tenant A"]
        BO --> BIN["INBOUND access ← the active tab here"]
        BIN --> BIU["B2B collaboration → Users and groups<br/>which A users/groups may enter"]
        BIN --> BIA["B2B collaboration → Applications<br/>which B apps they may reach"]
        BIN --> BIT["TRUST SETTINGS<br/>accept A's MFA claim<br/>accept A's compliant-device claim<br/>accept A's hybrid-joined claim"]
        BO --> BOUT["OUTBOUND access<br/>(not used in this direction)"]
        BO --> BCL["Microsoft cloud settings"]
    end
...
Two rules that follow from the shape of this blade:

1. **Access is an AND, not an OR.** A's outbound settings *and* B's inbound settings must both permit the user and the application. Either side can veto unilaterally. Neither side can grant unilaterally.
2. **Trust settings exist only on the inbound tab and are independent of access.** B can admit A's users while trusting none of A's claims — that is in fact the default. Admitting a partner does not imply believing their security assertions.

### 3.2 Default state before you configure anything

| Setting | Default | Consequence |
|---|---|---|
| Inbound B2B collaboration | **Allowed** — all external users, all applications | Any redeemed guest can reach any app in B unless CA or app assignment stops them |
| Outbound B2B collaboration | **Allowed** — all your users, all external applications | A's users can be invited into any tenant |
| B2B direct connect (inbound and outbound) | **Blocked** | Not relevant to this scenario, but confirm it stays blocked |
| Trust MFA from external tenants | **Not trusted** | Guests must register MFA methods in B |
| Trust compliant devices | **Not trusted** | A CA policy in B requiring compliance will hard-fail for guests |
| Trust hybrid-joined devices | **Not trusted** | Same |
| Microsoft cloud settings | **No clouds enabled** | Cross-cloud B2B fails until both sides opt in |

Adding an organizational entry for a partner does **not** by itself change behavior — a new entry inherits the default settings until you switch each tab to "Customize settings."
...
sequenceDiagram
    ...
    U->>A: Credentials + device PRT (Primary Refresh Token)
...
|---|---|---|
| **CTAS** | Cross-Tenant Access Settings | Entra admin center → External Identities → Cross-tenant access settings. Governs which external tenants your users reach and which external users reach you. Has Default settings and per-partner Organizational settings, each with Inbound access, Outbound access, and Microsoft cloud settings tabs. See §3.1 |
| **Trust settings** | — | Sub-section of the **Inbound** CTAS tab only. Determines whether you accept an external tenant's MFA, compliant-device, and Entra hybrid-joined claims. Independent of whether you allow access |
| **CA** | Conditional Access | Policy engine evaluated at token issuance in the resource tenant. Protocol-agnostic — applies identically to SAML and OIDC |
| **TRv2** | Tenant Restrictions v2 | Egress control enforced on your network edge or your managed devices, restricting which *foreign* tenants your users may sign into. Generates **no sign-in log** in the tenant that configures it |
| **Microsoft cloud settings** | — | CTAS tab enabling B2B across Microsoft cloud boundaries (commercial ↔ Azure Government ↔ 21Vianet) |
| **External collaboration settings** | — | Separate blade controlling who may invite guests and the domain allow/block list for invitations |
| **Entitlement management** | — | Entra ID Governance feature for time-bound, reviewable guest access packages |

### Identity and protocol

| Term | Expansion | Meaning in this document |
|---|---|---|
| **Home tenant** | — | Entra-A. Owns the user identity, credentials, and device object |
| **Resource tenant** | — | Entra-B. Owns App-B, the guest object, and issues the token App-B consumes |
| **B2B** | Business-to-Business | Microsoft Entra External ID collaboration between two workforce tenants |
| **HRD** | Home Realm Discovery | Entra resolving a submitted UPN to its home tenant and redirecting authentication there |
| **IdP** | Identity Provider | The tenant performing primary authentication — Entra-A here |
| **SP** | Service Provider | SAML term for the application consuming an assertion — App-B |
| **RP** | Relying Party | OIDC term for the application consuming an `id_token` — App-B |
| **STS** | Security Token Service | The Entra endpoint issuing tokens (`login.microsoftonline.com`) |
| **SAML** | Security Assertion Markup Language | XML-based SSO protocol. Identity delivered in a signed assertion via NameID and claims |
| **OIDC** | OpenID Connect | OAuth 2.0-based SSO protocol. Identity delivered in a JWT `id_token` |
| **UPN** | User Principal Name | Sign-in identifier. Guests in B take the form `user_a.com#EXT#@tenantB.onmicrosoft.com` |
| **NameID** | — | SAML's primary subject identifier. Default source is `user.userprincipalname`, which breaks for guests — see §7 |
| **`#EXT#`** | — | Marker embedded in a guest's UPN in the resource tenant, indicating an external identity |

### Session, device, and evaluation

| Term | Expansion | Meaning in this document |
|---|---|---|
| **PRT** | Primary Refresh Token | Device-bound token issued to an Entra joined/registered device, carrying device state into authentication |
| **CAE** | Continuous Access Evaluation | Near-real-time token revocation. Applies only to CAE-capable Microsoft resources — **not** to SAML SPs or arbitrary third-party OIDC apps |
| **MFA** | Multifactor Authentication | Asserted to the resource tenant via the `multipleauthn` claim |
| **`isCompliant`** | — | Device claim asserting Intune compliance in the home tenant. Only honored by B if **Trust compliant devices** is enabled |
| **Device extension attributes** | — | `extensionAttribute1`–`15` on the device object. The supported mechanism for cross-tenant CA device filtering — see §6 |
| **Sign-in frequency** | — | CA control forcing reauthentication. Effective only at the next authentication request, not mid-session |
| **Report-only mode** | — | CA policy state that logs evaluation results without enforcing. Mandatory first step for any guest-targeting policy |

### Logging and diagnostics

| Term | Expansion | Meaning in this document |
|---|---|---|
| **`crossTenantAccessType`** | — | Sign-in log field. Values include `none`, `b2bCollaboration`, `b2bDirectConnect`, `microsoftSupport`, `serviceProvider`, `passthrough` |
| **Home tenant ID / Resource tenant ID** | — | Sign-in log fields. The home tenant *name* is deliberately not populated cross-tenant; correlate by GUID |
| **AADSTS** | Azure Active Directory Security Token Service | Prefix on Entra authentication error codes |
| **KQL** | Kusto Query Language | Query language for Log Analytics / Microsoft Sentinel `SigninLogs` |
| **MAU** | Monthly Active Users | Billing model for External ID guest identities |

### Not this scenario — disambiguation

| Term | Why it is not this |
|---|---|
| **B2B direct connect** | No guest object is created; trust is mutual and app-scoped (Teams shared channels). Blocked by default both directions. Unsupported across Microsoft clouds |
| **Cross-tenant synchronization** | A provisioning mechanism that *creates* B2B objects in the resource tenant. Complementary to, not a substitute for, CTAS |
| **External tenants / CIAM** | Formerly Azure AD B2C. Customer-facing identity, a different tenant type and product surface |
| **Federation / forest trust** | Active Directory concept. No equivalent exists here — no trust object, no transitivity, no Kerberos, no SID history |
