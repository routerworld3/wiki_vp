...
After this section the document uses **home tenant** and **resource tenant** exclusively.
...
## 3. "One-way trust" is the wrong model

There is **no trust object** in Entra. Nothing here resembles an Active Directory forest trust. Access from A to B is the intersection of **four independent switches spread across two policy objects**, each owned by a different tenant:
...
sequenceDiagram
    ...
    Note over A,B: Gate 1 - cross-tenant access<br/>A outbound to B AND B inbound from A
    B->>BR: Redirect to Entra-A for authentication
    BR->>A: Authenticate home identity
    A->>A: Evaluate A Conditional Access on the auth event
    ...
    App->>App: Validate issuer and tid equal B, then apply app roles
...
- **Cross-tenant access settings gate the flow before the redirect to A** (Gate 1), not after. A blocked inbound or outbound setting fails the sign-in without A ever being reached.
...
| `idp` | A's issuer URI | The only in-token evidence of the home tenant |
| `preferred_username` / `upn` | `user_contoso.com#EXT#@tenantB.onmicrosoft.com` | Breaks naive email matching and string-based SIEM correlation |
| SAML `NameID` | Issued by B per the enterprise app's claim configuration | Same caveat as `oid` |

**SIEM implication:** to correlate an A-side authentication event with a B-side resource event, join on **home tenant ID plus `idp`**, not on `oid`, `sub`, or UPN. The identifiers are deliberately different on each side.
...
**Both tenants evaluate CA, at different points in the flow:**

| Tenant | What it evaluates | What it cannot do |
|---|---|---|
| **A (home)** | The authentication event. Policies scoped to *All resources* / *All cloud apps* fire here | Cannot generally target App-B by app ID. Scoping which foreign tenants A's users may reach is done with **outbound cross-tenant access settings** plus **tenant restrictions v2** |
